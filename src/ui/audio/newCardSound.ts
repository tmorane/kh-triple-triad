...
export function playNewCardSound() {
  try {
    ...
    // iOS/Safari can suspend the context until a user interaction resumes it.
    if (context.state === 'suspended') {
      void context.resume()
    }

    const startAt = context.currentTime + 0.01
    scheduleTone(context, startAt, 659.25, 0.12, 0.045)
    scheduleTone(context, startAt + 0.1, 783.99, 0.14, 0.04)
    scheduleTone(context, startAt + 0.2, 987.77, 0.18, 0.038)
  } catch {
